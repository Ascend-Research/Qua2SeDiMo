...
from bops.annotate_bops_sdxl import annotate_bops
...
def main():
    ...
    parser.add_argument("--multi_chkpt", type=str, default="multiquantizers/sdxl_mq.pt")
    ...
    model = StableDiffusionXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", torch_dtype=torch.float16, use_safetensors=True, variant="fp16").to("cuda") #, torch.float)
    #if opt.refiner:
    refiner = StableDiffusionXLImg2ImgPipeline.from_pretrained("stabilityai/stable-diffusion-xl-refiner-1.0", torch_dtype=torch.float16, use_safetensors=True, variant="fp16", vae=model.vae).to("cuda")
    ...
    with torch.no_grad():
        model(prompt=["weight calibration prompt"], height=512, width=512)
    ...
    for n in tqdm(range(opt.num_archs), desc="configs"):
        ...
        for i in tqdm(range(0, len(data), batch_size), desc="data"):
            ...
            image = model(prompt=prompts, num_inference_steps=40, denoising_end=0.8,
                            height=512, width=512, output_type="latent").images
            image = refiner(prompt=prompts, num_inference_steps=40, denoising_start=0.8, image=image).images

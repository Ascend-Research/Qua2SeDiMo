        ...
        for i in range(iters):
            if isinstance(cached_inps, list):
                ...
                if len(cached_inps) == 7:
                    ...
                    cur_am = cached_inps[1][idx].to(device) if type(cached_inps[1]) == torch.Tensor else None
                    cur_ehs = cached_inps[2][idx].to(device) if type(cached_inps[2]) == torch.Tensor else None
                    cur_eam = cached_inps[3][idx].to(device) if type(cached_inps[3]) == torch.Tensor else None
                    ...
                    cur_inp = (cur_x, cur_am, cur_ehs, cur_eam, cur_ts, cur_cak, cur_cl)
                else:
